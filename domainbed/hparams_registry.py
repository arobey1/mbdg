...
def _hparams(algorithm, dataset, random_seed, test_envs):
    """
    ...
    if algorithm in ['DANN', 'CDANN']:
        ...
        _hparam('mbdg_gamma', 0.025, lambda r: r.uniform(0.0001, 0.01)) # 0.01
    ...
    if dataset in SMALL_IMAGES:
        ...
        _hparam('batch_size', 16, lambda r: int(2**r.uniform(3, 4.5)))
    ...
    if algorithm in ['MBDG', 'MBDG_Reg', 'MBDG_DA', 'MBDA']:
...
def default_hparams(algorithm, dataset, test_envs):
    return {a: b for a, (b, c) in _hparams(algorithm, dataset, 0, test_envs).items()}
